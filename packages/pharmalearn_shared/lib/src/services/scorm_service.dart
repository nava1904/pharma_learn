...
class ScormService {
  ...
  static const String _bucket = 'scorm-content';
  ...
  Future<ScormManifest> processPackage({
    ...
    final manifestFile = archive.files.firstWhere(
      (f) => f.name.toLowerCase() == 'imsmanifest.xml',
      orElse: () => throw ScormValidationException(
        'Invalid SCORM package: imsmanifest.xml not found at root',
      ),
    );
    ...
    final hasLaunchFile = archive.files.any(
      (f) => f.name == manifest.launchUrl || f.name == '/${manifest.launchUrl}',
    );
  ...
  ScormManifest _parseManifest(String xml) {
    ...
    // Namespace handling - SCORM 1.2 uses various namespaces
    // We'll search without namespace prefix for compatibility
    ...
    if (resources != null) {
      for (final resource in resources.findAllElements('resource')) {
        // Check for scormtype or scormType (case varies)
  ...
  /// Gets signed URLs for the launch page and all referenced resources.
  Future<ScormLaunchUrls> getLaunchUrls({
    ...
    // The signed URL includes the full path, so we extract the base
  ...
  Future<void> deletePackageContent({
    ...
    final files = await _supabase.storage.from(_bucket).list(path: path);
